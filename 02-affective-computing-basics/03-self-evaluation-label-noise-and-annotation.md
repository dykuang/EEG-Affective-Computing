...
Each method introduces different types and amounts of noise.
...
- **Correlation** between raters on dimensional scales

Typical findings: inter-rater agreement on emotion is moderate at best, and often lower for subtle or mixed emotions.
...
1. **Performance ceiling**: if labels are unreliable, no model can achieve perfect accuracy — the Bayes error rate is elevated.
...
- Use **label smoothing** to soften hard targets
...
- Report **inter-rater agreement** as an upper bound on expected performance
