...
EMOD is especially relevant when source and target datasets use different emotion taxonomies or a mixture of discrete and dimensional annotations—the setting in which label identity alone is an inadequate transfer signal. Its V–A geometry can be viewed as a softer alternative to forcing one-to-one label correspondence; it also suggests a natural initialization for the domain adaptation and few-shot calibration procedures discussed below. Because its broad pretraining and reusable encoder are foundation-model-adjacent rather than simply dataset-specific transfer, see [Chapter 11, Section 03: EEG Foundation Models](../11-special-topics/03-eeg-foundation-models.md) for the broader design criteria, adaptation mechanisms, and limitations of EEG foundation models.
